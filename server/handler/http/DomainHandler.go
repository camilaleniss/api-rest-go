...
import (
	...
	"github.com/camilaleniss/api-rest-go/server/model"
	repository "github.com/camilaleniss/api-rest-go/server/repository"
	domain "github.com/camilaleniss/api-rest-go/server/repository/domain"
...
func GetDomainsResponse(domainsbd []connection.DomainBD) model.DomainResponse {
	var hosts []model.HostResponse
	for i := 0; i < len(domainsbd); i++ {
		domain := model.HostResponse{
			Host: domainsbd[i].Host,
		}
